...
path_to_run_fld_1 = os.path.join(WORKSPACE_PATH, 'ROMNet/Rect_100Instants_TransRotScale/DeepONet/Run_18/')
...
sys.path.insert(0, path_to_run_fld_1)

from ROMNet_Input import inputdata

InputData_1 = inputdata(WORKSPACE_PATH)
...
Net_1                        = getattr(rmnt.nn, surrogate_type)

model_1                      = rmnt.model.Model_TF(InputData_1)

if (InputData_1.phys_system is not None):
    System_1 = getattr(rmnt.pinn.system, InputData_1.phys_system)
    system_1 = System_1(InputData_1)
    
model_1.build(InputData_1, None, Net_1, system_1)#, loadfile_no='000027')

sys.path.remove(path_to_run_fld_1)
...
sys.path.insert(0, path_to_run_fld_2)

from ROMNet_Input import inputdata

InputData_2 = inputdata(WORKSPACE_PATH)
...
Net_2                        = getattr(rmnt.nn, surrogate_type)

model_2                      = rmnt.model.Model_TF(InputData_2)

if (InputData_2.phys_system is not None):
    System_2 = getattr(rmnt.pinn.system, InputData_2.phys_system)
    system_2 = System_2(InputData_2)
    
model_2.build(InputData_2, None, Net_2, system_2)#, loadfile_no='000027')

sys.path.remove(path_to_run_fld_2)
...
#axs     = plt.figure(constrained_layout=True, figsize=(10,8)).subplots(1, 2, sharey=True)
...
for it, t in enumerate(tVec):
    ...
    axs[1].text( -9.5,  8.5, r'Test Predictions', color=ColorVec[1], fontsize=26, weight='bold')
    axs[1].text(-14.5, 13.5, r'Extrapolations', color=ColorVec[1], fontsize=26, weight='bold')

    im3       = axs[2].imshow((yMat_2).reshape(Nx,Ny), animated=True, origin='lower', cmap=cm.turbo, extent=([xMin, xMax, yMin, yMax]))
    rect      = plt.Rectangle((-10.,-10.), 20., 20., linewidth=2, edgecolor=ColorVec[1], facecolor='none')
    axs[2].add_patch(rect)
    axs[2].text( -9.5,  8.5, r'Test Predictions', color=ColorVec[1], fontsize=26, weight='bold')
    axs[2].text(-14.5, 13.5, r'Extrapolations', color=ColorVec[1], fontsize=26, weight='bold')

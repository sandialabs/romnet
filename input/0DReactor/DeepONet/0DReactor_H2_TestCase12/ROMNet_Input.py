...
class inputdata(object):
    ...
    def __init__(self, WORKSPACE_PATH):
        ...
        for i in range(self.n_branches):
           ...
           self.structure['DeepONet']['Decoder_'+str(i+1)] = ['Main']
        ...
        self.n_neurons           = {'DeepONet': {'Branch': {'Main': np.array([16,16,16,self.n_branch_out])},  
                                                'Stretch': {'Main': np.array([16,16,16,self.n_trunks])},
                                                  'Trunk': {'Main': np.array([16,16,16,self.n_trunk_out])},
                                                'Decoder': {'Main': np.array([self.n_branch_out,8,8,1])}}}   # Dictionary Containing the No of Neurons for each Layer
        self.act_funcs           = {'DeepONet': {'Branch': {'Main': ['tanh','tanh','tanh','linear']},  
                                                ...
                                                'Decoder': {'Main': ['relu','relu','relu','linear']}}}       # Dictionary Containing the Activation Funct.s for each Layer
        self.dropout_rate        = {'DeepONet': {'Branch': {'Main': None},  
                                                'Stretch': {'Main': None},  
                                                  'Trunk': {'Main': None},
                                                'Decoder': {'Main': None}}}                                  # Dictionary Containing the Dropout Rate for each Sub-Component
        self.dropout_pred_flg    = {'DeepONet': {'Branch': {'Main': False},  
                                                'Stretch': {'Main': False},
                                                  'Trunk': {'Main': False},
                                                'Decoder': {'Main': False}}}                                 # Dictionary Containing the Dropout-at-Prediction Flag for each Sub-Component 
        self.softmax_flg         = {'DeepONet': {'Branch': {'Main': False},   
                                                'Stretch': {'Main': False},
                                                  'Trunk': {'Main': False},
                                                'Decoder': {'Main': False}}}                                 # Dictionary Containing the Softmax Flag for each Sub-Component 
